...
export function useAtlasNextEvent(): AtlasNextEvent | null {
  ...
  const query = useQuery({
    ...
    queryFn: async (): Promise<AtlasNextEvent | null> => {
      ...
      // Two source paths: regattas owned by the user (`created_by`), and
      // user-created race_events (different table used by the add-race
      // flow). Pick whichever has the earliest future start time.
      const [regattaRes, raceEventRes] = await Promise.all([
        supabase
          .from('regattas')
      ...
      const regattaCandidate =
        regattaRes.error || !regattaRes.data
          ? null
          : mapRegattaToNextEvent(regattaRes.data as RegattaRow);
      const raceEventCandidate =
        raceEventRes.error || !raceEventRes.data
          ? null
          : mapRaceEventToNextEvent(raceEventRes.data as RaceEventRow);

      if (!regattaCandidate) return raceEventCandidate;
      if (!raceEventCandidate) return regattaCandidate;

      // Both present — return whichever has the earlier ISO timestamp.
      const regattaTs = regattaRes.data?.start_date ?? '';
      const raceEventTs = raceEventRes.data?.start_time ?? '';
      return regattaTs < raceEventTs ? regattaCandidate : raceEventCandidate;
    },
