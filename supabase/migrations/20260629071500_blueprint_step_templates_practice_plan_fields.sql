-- Studio step templates need the same structured planning fields as Practice
-- add-step without turning each optional field into a nullable column.

ALTER TABLE public.blueprint_step_templates
  ADD COLUMN IF NOT EXISTS plan_metadata jsonb NOT NULL DEFAULT '{}'::jsonb;

COMMENT ON COLUMN public.blueprint_step_templates.plan_metadata IS
  'Practice-compatible plan fields for Studio-authored template steps: why, when_label, where_label, beats, and related optional context.';
