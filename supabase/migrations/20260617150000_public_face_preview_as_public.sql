-- "Preview as public" for the public-face owner.
--
-- The owner normally bypasses every section flag and per-step check (v_is_self).
-- That makes a self-view useless for previewing what others see. This adds a
-- defaulted `preview_as_public` argument: when true, the RPC treats the caller
-- as a non-related stranger —
--   * v_is_self := false, so the section flags (show_working_on_now,
--     show_capabilities, show_practice_circle) apply, and
--   * a nil step-viewer is used for person_step_visible_to(), so the owner's
--     own private/crew/fleet steps stop counting (the owner would otherwise
--     match the `ts.user_id = p_viewer` short-circuit and over-show).
--
-- preview_as_public can only ever return *less* data than the default, so it is
-- safe to expose to any authenticated caller (no new disclosure path).
--
-- The single-arg overload is dropped so PostgREST resolves the call to the new
-- two-arg function unambiguously.

DROP FUNCTION IF EXISTS public.get_person_public_face(uuid);
