-- Public-face capability rows want a real first-person evidence quote +
-- provenance (the mock's defining feature). The capture/reflection prose the
-- learner already wrote was gathered for tagging but never persisted, so the
-- public face had only a count. Store a verbatim quote + a derived provenance
-- on each evidence row, then surface the strongest one per capability.
...
-- ---------------------------------------------------------------------------
-- 2. get_person_public_face — capabilities aggregate now returns the strongest
--    row's quote + provenance. Quote may be NULL (older rows / no prose); the
--    UI drops the line when absent. Concept + circle blocks unchanged.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- evidence/provenance come from the strongest, most recent row that
  -- actually carries a quote (FILTER drops rows with none).
