-- Add optional restrict_user_ids[] arg to atlas_peer_steps_near so the
-- chip-row contextual-group filter narrows at the RPC level rather than
-- relying on client-side filtering. Big-org case: a JHSON cohort of 8
-- doesn't need the full 1000-row peer scan walked back to 8 in JS.
--
-- When restrict_user_ids is null/empty, behavior is unchanged.
