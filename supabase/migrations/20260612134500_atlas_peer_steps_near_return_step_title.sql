-- atlas_peer_steps_near: return the timeline step title with every visible peer
-- row. The RPC is already the authority for whether a viewer may see a peer
-- step on Atlas; returning the title from the same SECURITY DEFINER path avoids
-- a second client-side timeline_steps lookup that can disagree with the map
-- visibility predicate and fall back to "Dropped pin (...)".
...
BEGIN
  ...
  SELECT l.step_id,
    ...
    NULLIF(trim(ts.title), '') AS step_title
  FROM labeled l
  LEFT JOIN public.profiles pr ON pr.id = l.set_by
  LEFT JOIN public.timeline_steps ts ON ts.id = l.step_id;
END;
$$;
