-- Extend atlas_peer_steps_near to include affinity_group_members in the
-- relationship CASE. Mapping:
--   crew_pod        → 'crew'   (private crew groups light the Crew chip)
--   cohort          → 'cohort' (cohort affinity_group OR atlas_are_cohort_peers)
--   class_fleet     → 'fleet'  (Dragon sailor at NYYC reads as fleet to RHKYC Dragon sailor)
--   practice_group  → 'fleet'  (Saturday radial squad reads as fleet)
