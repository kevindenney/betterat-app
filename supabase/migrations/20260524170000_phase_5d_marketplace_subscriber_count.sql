-- Phase 5d · expose active subscriber count for marketplace social proof
--
-- Rating is one signal, but it lags new listings (no reviews = empty
-- box) and is gameable in small samples. Active subscriber count is a
-- second, complementary social-proof signal. Both RPCs now compute the
-- count of subscriptions with status in ('active', 'trialing').
--
-- Privacy: only the aggregate count is exposed. No buyer identities,
-- emails, or activity dates leak through these RPCs.
