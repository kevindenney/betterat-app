-- atlas_peer_steps_near now post-processes preview_name: when the
-- stored step_location.name is coord-only ("Dropped pin (22.36, 114.27)"
-- or a bare "(lat, lng)" pair), swap it for the nearest named place
-- within 1km via the nearest_named_place RPC. Atlas pin tooltips then
-- read "Hebe Haven" instead of raw coords without any client-side work.
