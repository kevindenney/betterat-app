...
| N | PRIORITY-HIGH production privacy fix: make `profiles.profile_public` default false for new users, migrate existing profiles to private, and guard public profile route fallback behavior. | `specs/PHASE_N_PROFILE_PUBLIC_DEFAULT_FIX_SPEC.md`, B.12 investigation `64f3f6ec` | none | spec-written | 3 | database snapshot before migration; public-route privacy guard | none; privacy fix |
...
| `PHASE_N_PROFILE_PUBLIC_DEFAULT_FIX_SPEC.md` | Production privacy fix: `profiles.profile_public` default false, migrate existing rows private, app defaults false, route guard. | written; execute before May 20 |
