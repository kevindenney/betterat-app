...
2. **Step completion beat** (§7) — ✅ *partially resolved.* The wired-but-dead blueprint trophy now fires (gate was checking `'completed'`; completion produces `'settled'`), and every completion gets a success haptic. Net-new visual beat for non-blueprint steps remains.
...
2. **Step completion** (`project_critique_completion_aha`) — ✅ **PARTIALLY RESOLVED.** Root cause found: `StepCompleteCelebration` *was* fully built and wired in `StepDetailContent.tsx`, but its render gate (`showCelebration`) tested `status === 'completed'` while the Done action only ever produces `'settled'` — so the trophy was dead code. Fixed the gate to fire on the settled boundary, and added `hapticSuccess()` to `handleToggleDone` so **every** completion (blueprint or not) now delivers a tactile beat at the moment of settle. *Code-verified (typecheck + lint clean); on-device-unverified* — the blueprint trophy UI requires completing a blueprint-adopted step in-sim, and idb taps weren't registering on the signed-in sim. **Remaining (net-new):** a lightweight *visual* celebration for non-blueprint steps (the haptic fires for all, but the trophy card is still blueprint-only).
